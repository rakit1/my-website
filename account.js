class AccountPage {
    ...
    displayUserProfile() {
        ...
        const userRole = this.user.role || 'Игрок';
    ...
    subscribeToTicketCount() {
        ...
        const query = ticketsRef.where('is_closed', '==', false);

        this.unsubscribe = query.onSnapshot(snapshot => {
            ticketCountElement.textContent = snapshot.size;
        }, error => {
            console.error("Ошибка real-time подписки на тикеты:", error);
    ...
    async fetchAndDisplayTickets() {
        ...
        try {
            const ticketsRef = firebase.firestore().collection('tickets');
            const query = ticketsRef.where('user_id', '==', this.user.uid).orderBy('created_at', 'desc');
