class AuthManager {
    constructor() {
        ...
        this.auth.onAuthStateChanged(async (firebaseUser) => {
            if (firebaseUser) {
                // ИСПРАВЛЕНИЕ БАГА №1: Принудительно закрываем модальное окно после входа
                const authModal = document.querySelector('#authPage');
            ...
            // Сообщаем всему сайту, что информация о пользователе готова
    ...
    async signInWithDiscord() {
        ...
        // ИСПРАВЛЕНИЕ БАГА №4: Используем signInWithRedirect
        try {
            await this.auth.signInWithRedirect(provider);
        } catch (error) {
            console.error("Ошибка входа:", error);
            alert("Не удалось войти через Discord. Код ошибки: " + error.code);
        }
    }
    ...
    updateUserUI(user) {
        ...
        if (user) {
            const name = this.user?.username || user.displayName || 'Пользователь';
            const avatarUrl = this.user?.avatar_url || user.photoURL;
